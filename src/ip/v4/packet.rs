...
impl<B: AsRef<[u8]> + AsMut<[u8]>> Packet<B> {
    ...
    pub fn set_id(&mut self, value: u16) -> Result<&mut Self> {
        Cursor::new(&mut self.buffer.as_mut()[4..]).write_u16::<BigEndian>(value)?;
    ...
    pub fn set_flags(&mut self, value: Flags) -> Result<&mut Self> {
        Cursor::new(&mut self.header_mut()[6..]).write_u16::<BigEndian>(value.bits())?;
    ...
    pub fn set_checksum(&mut self, value: u16) -> Result<&mut Self> {
        Cursor::new(&mut self.header_mut()[10..]).write_u16::<BigEndian>(value)?;
...
mod test {
    ...
    fn values() {
        let raw = [
            0x45u8, 0x00, 0x00, 0x34, 0x2d, 0x87, 0x00, 0x00, 0x2c, 0x06, 0x5c, 0x74, 0x42, 0x66,
            0x01, 0x6c, 0xc0, 0xa8, 0x00, 0x4f,
        ];
    ...
    fn owned() {
        let raw: Vec<u8> = vec![
            0x45, 0x00, 0x00, 0x34, 0x2d, 0x87, 0x00, 0x00, 0x2c, 0x06, 0x5c, 0x74, 0x42, 0x66,
            0x01, 0x6c, 0xc0, 0xa8, 0x00, 0x4f,
        ];
        let ip = ip::v4::Packet::no_payload(raw).unwrap();
